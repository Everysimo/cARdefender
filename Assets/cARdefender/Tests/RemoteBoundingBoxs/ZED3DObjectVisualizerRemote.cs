...
public class ZED3DObjectVisualizerRemote : MonoBehaviour
{
    ...
    /// </summary>
    ///
    public bool IsVector3NaN(Vector3 input)
    {
    ...
    private void Visualize3DBoundingBoxes()
    {
        ...
        {
            ObjectRecognized objectRecognized = boxListener.ObjectRecognizeds[i];
            Bounds objbounds = objectRecognized.GetBounds(boxListener.camRotation);
            ...
            if (!IsVector3NaN(obj_position))
            {
                bbox.transform.position = obj_position;

                bbox.transform.rotation = objectRecognized.Get3DWorldRotation(boxListener.camRotation); //Rotate them. 
            }
    ...
    private BoxInformationContainer GetBBoxForObject(int id)
    {
        if (!liveBBoxes.ContainsKey(id))
        {
            ...
            newbox.name = "Object #" + id;


            Color col;
            if (idColorDict.ContainsKey(id))
            {
                col = idColorDict[id];
            }
            else
